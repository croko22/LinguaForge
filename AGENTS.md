...
## Architecture
- 5-layer: `handler → service → {parser, storage, repository}`
...
| `DocumentRepository` | `repository` | `Create`, `GetByID`, `List`, `UpdateStatus`, `UpdateMetadata` |
...
## Dev workflow
- New endpoint: handler method → service method → repo method (if needed)
